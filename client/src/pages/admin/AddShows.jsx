...
const AddShows = () => {
    ...
    const handleDateTimeAdd = () => {
        ...
        if (!date || !time) return;
        setDateTimeSelection((p) => {
            const times = p[date] || [];
            if (!times.includes(time)) {
                return { ...p, [date]: [...times, time] };
            }
            return p;
        })
    }

    const handleRemoved = (date, time) => {
        setDateTimeSelection((p) => {
            const filteredTimes = p[date].filter((t) => t !== time);
    ...
    return nowPlayingMovies.length > 0 ? (
        < >
            ...
            <div className='mt-6'>
                ...
                <div className='inline-flex  gap-5 border border-gray-600 pl-3 p-1 py-2 rounded-lg'>
                                ...
                                <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                                    {times.map((time) => (
                                        <div key={time} className='border border-primary px-2 py-1 flex items-center rounded'>
                                            <span>{time}</span>
                                            <DeleteIcon onClick={() => handleRemoved(date, time)} width={15} className='ml-2 text-red-500 hover:text-red-700 cursor-pointer' />
                                        </div>
                                    ))}
